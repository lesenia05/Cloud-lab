...
def create_jwt(project_id, private_key_file, algorithm):
    ...
    print('Creating JWT using {} from private key file {}'.format(
        jwt.encode(token, private_key, algorithm=algorithm), private_key_file))
...
class Device(object):
    ...
    def __init__(self):
        self.temperature = 0
        self.attributes = {"id": "device1", "location": [51.38555, -120.16730], "time": str(datetime.datetime.utcnow())}
        self.fan_on = False
...
def main():
    ...
    for _ in range(args.num_messages):
        ...
        # as a JSON string.
        payload = json.dumps({'temperature': device.temperature, "attributes": device.attributes})
